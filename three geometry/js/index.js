...
function cutovereventgroupfunc1() {
    var camera, scene, renderer, orbitControls, clock, delta;
    var data = { "result": { "data": [{ "vendorname": "JUNIPER", "num": 30 }, { "vendorname": "华为", "num": 50 }, { "vendorname": "思科", "num": 20 }] } }
    $(".cutovereventgroup1").html('<div id="cutovereventgroup1" style="width:100%;height:100%"></div>')
    var result = data.result.data
    function compare(property) {
        return function (a, b) {
            var value1 = a[property];
    ...
    function main(result) {
        scene = new THREE.Scene();
        var geometry1 = new THREE.CylinderBufferGeometry(200, 200, 100, 32, 1, false, 0 * Math.PI / 180, 360 * 0.3 * Math.PI / 180);
        var material1 = new THREE.MeshBasicMaterial({ color: '#3BDCF6' });
        var cylinder1 = new THREE.Mesh(geometry1, material1);
        cylinder1.position.set(0, 0, 0)
        scene.add(cylinder1);

        var geometry2 = new THREE.CylinderBufferGeometry(200, 200, 100, 32, 1, false, 360 * 0.3 * Math.PI / 180, 360 * 0.5 * Math.PI / 180);
        var material2 = new THREE.MeshBasicMaterial({ color: '#fd947e' });
        var cylinder2 = new THREE.Mesh(geometry2, material2);
        cylinder2.position.set(0, 0, 0)
        scene.add(cylinder2);

        var geometry3 = new THREE.CylinderBufferGeometry(200, 200, 100, 32, 1, false, 360 * 0.8 * Math.PI / 180, 360 * 0.2 * Math.PI / 180);
        var material3 = new THREE.MeshBasicMaterial({ color: '#4A5BE7' });
        ...
        var ambient = new THREE.AmbientLight(0x444444);
        scene.add(ambient);
        var width = $("#cutovereventgroup1").width()
        var height = $("#cutovereventgroup1").height()
        var k = width / height; //窗口宽高比
        var s = 100; //三维场景显示范围控制系数，系数越大，显示的范围越大
        //创建相机对象
        camera = new THREE.OrthographicCamera(width / - 2, width / 2, height / 2, height / - 2, 1, 10000);
